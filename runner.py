...
def clone_repo(github_url, local_path):
    try:
        if not os.path.exists(local_path):
            result = subprocess.run(['git', 'clone', github_url, local_path], check=True, capture_output=True, text=True)
...
class Runner:
    ...
    def generate_hierachy(self):
        ...
        logger.info("Starting to generate project structure.")
        ...
        if not repo_structure:
            logger.warning("No files detected in the project structure. Please check file paths and ignore patterns.")
        else:
            logger.info(f"Detected project structure: {repo_structure}")
        ...
        logger.info(f"Project structure saved to {json_file}")
